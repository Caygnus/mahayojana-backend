...
export interface IPoliciesDocument
  extends Omit<Policies, 'toJSON' | 'toObject' | 'id'>,
    Document {
...
PoliciesSchema.pre('validate', function (next) {
  ...
  const schemaDefinition =
    doc.schemaDefinition instanceof Map
      ? Object.fromEntries(doc.schemaDefinition)
      : doc.schemaDefinition;

  const dynamicFields =
    doc.dynamicFields instanceof Map
  ...
  for (const [field, schema] of Object.entries(schemaDefinition)) {
    ...
    if (
      ...
      (dynamicFields[field] === undefined || dynamicFields[field] === null)
    ) {
    ...
    switch (fieldSchema.type) {
      case 'string':
        ...
        if (
          ...
          this.invalidate(
            `dynamicFields.${field}`,
            `${field} must be at least ${fieldSchema.minLength} characters`,
          );
        }
        ...
        if (
          ...
          this.invalidate(
            `dynamicFields.${field}`,
            `${field} must be at most ${fieldSchema.maxLength} characters`,
          );
        }
      ...
      case 'number':
        ...
        if (fieldSchema.step !== undefined && value % fieldSchema.step !== 0) {
          this.invalidate(
            `dynamicFields.${field}`,
            `${field} must be a multiple of ${fieldSchema.step}`,
          );
        }
        break;
      ...
      case 'array':
        ...
        if (
          ...
          this.invalidate(
            `dynamicFields.${field}`,
            `${field} must have at least ${fieldSchema.minItems} items`,
          );
        }
        ...
        if (
          ...
          this.invalidate(
            `dynamicFields.${field}`,
            `${field} must have at most ${fieldSchema.maxItems} items`,
          );
        }
  ...
  switch (schema.type) {
    ...
    case 'number':
      ...
      if (schema.min !== undefined && value < schema.min) {
        context.invalidate(
          `dynamicFields.${path}`,
          `${path} must be at least ${schema.min}`,
        );
      }
      ...
      if (schema.max !== undefined && value > schema.max) {
        context.invalidate(
          `dynamicFields.${path}`,
          `${path} must be at most ${schema.max}`,
        );
      }
      break;
...
export const PoliciesModel = mongoose.model<IPoliciesDocument>(
  'Policies',
  PoliciesSchema,
);
