...
const cleanupRoutes = () => {
    ...
    const versionDirs = fs.readdirSync(ROUTES_DIR)
        .filter(dir => dir.startsWith('v') && fs.statSync(path.join(ROUTES_DIR, dir)).isDirectory());
    ...
    versionDirs.forEach(version => {
        ...
        if (fs.existsSync(versionIndexPath)) {
            ...
            const importRegex = new RegExp(`import\\s+${featureNameKebab}Routes\\s+from\\s+['"]../../features/${featureNameKebab}/routes/${featureNameKebab}.routes['"];?\\n?`);
            content = content.replace(importRegex, '');

            // Remove route registration
            const routeRegex = new RegExp(`\\s*router\\.use\\s*\\(['"]/${featureNameKebab}['"]\\s*,\\s*${featureNameKebab}Routes\\);?\\n?`);
